...
class MakeFeeOperationGatewayAPI(OperationsGatewayAPI):
    def __init__(self):
        super().__init__()
        self.MAKE_FEE_OPERATION_API = f"{self.OPERATIONS_API}/make-fee-operation"

    @allure.step("Send POST request to make fee operation")
    def send_request(self, card_id: str, account_id: str):
        ...
        response = httpx.post(
            self.MAKE_FEE_OPERATION_API, json=operation_data.model_dump(by_alias=True)
        )
