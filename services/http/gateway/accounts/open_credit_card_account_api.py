...
class OpenCreditCardAccountGatewayAPI(AccountsGatewayAPI):
    def __init__(self):
        super().__init__()
        self.OPEN_CREDIT_CARD_ACCOUNT_API = (
            f"{self.ACCOUNTS_API}/open-credit-card-account"
        )

    @allure.step("Send POST request to open credit card account")
    def send_request(self, user_id: str):
        user_data = OpenAccountRequestSchema(user_id=user_id).model_dump(by_alias=True)
        response = httpx.post(self.OPEN_CREDIT_CARD_ACCOUNT_API, json=user_data)
